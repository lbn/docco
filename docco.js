...
(function() {
  ...
  parse = function(source, code, config) {
    var codeText, docsText, hasCode, i, isText, j, k, lang, len, len1, line, lines, match, maybeCode, multiline, save, sections;
    if (config == null) {
      config = {};
    }
    lines = code.split('\n');
    ...
    for (k = 0, len1 = lines.length; k < len1; k++) {
      line = lines[k];
      if (line.match(lang.mlStartMatcher) && !multiline) {
        multiline = true;
      } else if (line.match(lang.mlEndMatcher) && multiline) {
        multiline = false;
      } else if (multiline) {
        if (hasCode) {
          save();
  ...
  buildMatchers = function(languages) {
    ...
    for (ext in languages) {
      ...
      l.mlStartMatcher = RegExp("^\\s*" + l.mlstart + "\\s?");
      l.mlEndMatcher = RegExp("^\\s*" + l.mlend + "\\s?");
      l.commentFilter = /(^#![\/]|^\s*#\{)/;
    }
    return languages;
  };
